...
if __name__ == '__main__':
    ...
    shp_path = "D:/最新2021年全国行政区划/china_river_buffer.shp"
    zoom = 16
    save_path = "D:/20级光学影像/quanguo"
    ...
    data_num = [int(str(str(data_path).split("_")[-1]).split(".")[0]) for data_path in data_paths]
    data_num = np.array(data_num)

    pool = multiprocessing.Pool(multiprocessing.cpu_count())
    for i, coordinate in enumerate(selcet_coordinates):
        if np.in1d([i], data_num):
            continue
