...
class Downloader(Thread):
    ...
    def download(self, url):
        ...
        # # opener = ur.Request.build_opener(handler)
        # ur.Request.set_proxy(proxy)
...
# ---------------------------------------------------------
MAP_URLS = {
    "Google": "http://mts2.googleapis.com/vt?lyrs={style}&x={x}&y={y}&z={z}",
...
def main(left, top, right, bottom, zoom, save_path, i, style='s', server="Google China"):
    """
    ...
    try:
        filePath = save_path + f"shandong{i}.tif"
        ...
        print(f"图片 {i} 无 {zoom} 层级的图像")
        with open("gaza.txt", "a+") as file:
            file.write(str(i)+"/n")
    file.close()


def get_lenxy(x1, y1, x2, y2, z, source='google', style='s'):
    print(x1, y1, x2, y2)
    pos1x, pos1y = wgs_to_tile(x1, y1, z)
    pos2x, pos2y = wgs_to_tile(x2, y2, z)
...
def point_intersect_ploygon(coordinates, shp_path):
    data1, data2, data3, data4 = [], [], [], []
    for i, coordinate in enumerate(coordinates):
        data1.append([coordinate[0], coordinate[1]])
        data2.append([coordinate[2], coordinate[3]])
        data3.append([coordinate[0], coordinate[3]])
        data4.append([coordinate[2], coordinate[1]])

    def intersect(data, shp_path):
        data = pandas.DataFrame(np.array(data))
        gdf = gpd.GeoDataFrame(data, geometry= gpd.points_from_xy(data.loc[:, 0], data.loc[:, 1]))
        gaza = gpd.read_file(shp_path).to_crs("EPSG:4326")
        gaza = gpd.read_file(shp_path)
        result = gpd.sjoin(gdf, gaza, op='within')
        result = result[result['index_right'].notnull()].index.tolist()
        return result
    
    results = []
    results += intersect(data1, shp_path)
    results += intersect(data2, shp_path)
    results += intersect(data3, shp_path)
    results += intersect(data4, shp_path)
    return results

def get_coordinates(shp_path, zoom):
    ...
    for geom in gdf['geometry']:
        ...
        coordinate = [left, up, right, down]
        coordinates.append(coordinate)

    return coordinates

    data = point_intersect_ploygon(coordinates, shp_path)
    unique_values = np.unique(np.array(data))
    selcet_coordinates = []
    for unique_value in unique_values:
        selcet_coordinates.append(coordinates[unique_value])
    return selcet_coordinates
...
if __name__ == '__main__':
    ...
    shp_path = "C:\\Users\\14398\\Desktop\\shandong_all\\shandong_all.shp"
    zoom = 17
    save_path = "C:\\Users\\14398\\Desktop\\shandong_all\\img\\"
    os.makedirs(save_path, exist_ok=True)

    selcet_coordinates = get_coordinates(shp_path, zoom)
    # selcet_coordinates = [[123.153764865,41.75870841,123.377459,41.64635739]]
    print(f"筛选后的面个数为{len(selcet_coordinates)}")

    data_paths = os.listdir(save_path)
    data_num = [int(str(str(data_path).split("shandong")[-1]).split(".")[0]) for data_path in data_paths]
    # print(data_num)
    # print(len(data_num))
    data_num = np.array(data_num)

    pool = multiprocessing.Pool(multiprocessing.cpu_count())
    for i, coordinate in enumerate(selcet_coordinates):
        if np.isin([i], data_num):
            continue
        failed_download.append(i)
        pool.apply_async(func=main, args=(*coordinate, zoom, save_path, i, 's', "Google"))
    pool.close()
    pool.join()
    ...
    # # 保存下载失败的序号
    # failed_download = np.array(failed_download)
    # np.save('./geid_cl_download/failed_download.npy',failed_download)
