...
def exist_objs(list_1,list_2, sliceHeight, sliceWidth):
    ...
    '''
    return_objs=[]
    min_h, min_w = 100, 100  # 有些目标GT会被窗口切分，太小的丢掉
...
def make_slice_voc(outpath,exiset_obj_list,sliceHeight=1024, sliceWidth=1024):
    name = outpath.split(os.sep)[-1] # windows
...
def slice_im(List_subsets, outdir, raw_images_dir, raw_ann_dir, i=None, sliceHeight=640, sliceWidth=640,
             zero_frac_thresh=0.5, overlap=0.5, verbose=True):
    cnt = 0
    # print(List_subsets)
...
if __name__ == "__main__":
    ...
    raw_images_dir = 'D:\\data\\cfpp0919\\test\\image'   # 这里就是原始的图片
    raw_ann_dir = 'D:\\data\\cfpp0919\\test\\annoation'
    # 普通数据集格式
    # slice_voc_dir = 'D:\\data\\cfpp0912\\out\\test\\annotation'  # 切出来的标签也保存为voc格式
    # outdir = 'D:\\data\\cfpp0912\\out\\test\\img'
    
    # Yolo 数据集格式
    slice_voc_dir = 'D:\\data\\cfpp0919\\yolo0919\\labels\\test'  # 切出来的标签也保存为voc格式
    outdir = 'D:\\data\\cfpp0919\\yolo0919\\imagess\\test'
    ...
    if not_use_multiprocessing:
            slice_im(List_imgs, outdir, raw_images_dir, raw_ann_dir, sliceHeight=640, sliceWidth=640)
    else:
